package com.yibingo.race.security.config;


import com.yibingo.race.security.common.NoPasswordEncoder;
import com.yibingo.race.security.filter.JwtAuthenticationTokenFilter;
import com.yibingo.race.security.handler.*;
...
//todo 设置一下所有接口都要
               ...
               // .and().authorizeRequests().antMatchers(HttpMethod.GET, "/test").hasAuthority("test:list")
                ...
                .and().authorizeRequests().antMatchers("/login/test/wx").permitAll();
        //todo 上线的的时候这个记得加上
        //.and().authorizeRequests().antMatchers("/core/**").authenticated();//我自己加的：就是所有接口都要进行拦截鉴权
