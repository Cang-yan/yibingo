 ...
 */
@Service
public class TupleExtendsService {

    @Autowired
    TupleService tupleService;

    public List<Tuple> getListByFilter(TupleFilterMapper filterMapper) {

        QueryWrapper<Tuple> wrapper = new QueryWrapper<>();
        ...
        Page<Tuple> markPage = new Page<>(page, row);

        Page<Tuple> resultList = tupleService.page(markPage, wrapper);
