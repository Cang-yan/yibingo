 ...
 */
@Service
public class ImgTravelCardExtendsService {

    @Autowired
    ImgTravelCardService imgTravelCardService;

    public List<ImgTravelCard> getListByFilter(ImgTravelCardFilterMapper filterMapper) {

        QueryWrapper<ImgTravelCard> wrapper = new QueryWrapper<>();
        ...
        if (filterMapper.formRecordsId !=null) wrapper.eq("form_records_Id",filterMapper.formRecordsId);
        if (filterMapper.formRecordsIdIn !=null) wrapper.in("form_records_Id",filterMapper.formRecordsIdIn);
        ...
        Page<ImgTravelCard> markPage = new Page<>(page, row);

        Page<ImgTravelCard> resultList = imgTravelCardService.page(markPage, wrapper);
